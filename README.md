...
- 已有 run 缺少评论时，可运行 `python -m src.cli notes <run_name>`，在本地页面按 Query 导入 XLSX、编辑并保存；无需评分和模型配置。
- 页面保存的评论独立存放于 `results/<run>/human_notes.jsonl`，归因时覆盖匹配当前 Query/Answer/messages 版本的源数据评论；原始 `responses.jsonl` 不变。
...
- 旧 run 导入时已经丢失的评论不会被自动找回。可以使用人工评论页面，从含 Query/评论列的 XLSX 补录到当前 run。
...
python -m src.cli notes <run_name> --port 8766 --no-open
```

默认打开 `http://127.0.0.1:8765/notes`；省略 run 时选择最新实验。该服务仅监听本机，不加载 Judge/归因模型配置，不需要 API Key。现有 `report --serve` 页面也提供 `/notes` 入口。

1. **选择 XLSX**：第一行为表头，选择工作表、Query 列和人工评论列。文件最多 20 MB，解压后最多 100 MB，单表最多 50000 条数据、500 列。
2. **预览匹配**：优先原文匹配（统一换行、去首尾空白）；无匹配时，兼容 Unicode 重音编码、全角/半角/小问号、不可见分隔符及连续空白，再尝试忽略句首倒问号/句末问号。规范化和问句标点匹配会展示候选 Query 原文并要求手动勾选。保留大小写、重音区别和其他语义字符，不任意模糊匹配。重复、覆盖、未匹配及 Excel 长度上限都会明确告警。
3. **选择案例**：只有无告警的一对一匹配默认勾选。重复 Query 会展示各案例行号及回答摘录，由用户选择对应回答；表内多行指向同一案例时，必须只采用其中一行。空评论不会通过导入清空旧评论，公式/错误单元格需先转为文本值。
4. **编辑草稿**：将选中的评论加入草稿，可搜索 Query/评论/行号，筛选未填写、已填写、重复 Query、未保存或旧版评论。支持查看完整 Query、回答、上下文并逐条手动编辑。导入预览与草稿均不会立即写盘。
5. **保存**：点击“保存全部修改”，以 JSONL 原子保存并保留历史版本；手动清空文本再保存可清除有效评论。评论绑定当前 Query、Answer 和上下文，源内容变化后旧评论不自动套用，可在页面中核对并重新采用。

多页面同时编辑时，保存会检查加载时的版本；发现冲突则整批不保存，刷新仍保留草稿，用户可逐条采用已保存内容或保留草稿重新绑定当前版本。页面关闭前会提示尚未保存的修改。

保存后重新运行 `attribute` 即可读取最新评论；内容变化会使对应归因输入摘要变化并重新分析。历史评分和原始候选数据不会被修改。JSONL 完整保存长评论，Excel 本身已经截断的原始内容无法自动恢复。

Judge 报告、归因报告及人工评论页的搜索统一支持上述 Unicode/问号差异，并忽略大小写。`año` 与 `ano` 等重音不同的词不会被当作同一文本。匹配只生成查找键，不改写显示、复制或保存的原文。升级后请重启 `notes` 服务；已有 Judge HTML 需运行 `python -m src.cli report <run_name>` 重新生成（无需重新评分）。
...
默认评测十个维度（适用时为 1-5 分，不适用时为 `-`），可在 `eval.yaml` 的 `dimensions` 中自定义：
...
用户问题中可能包含请求时间信息（如 `[Current Time: 2024-06-01 14:30:00 CST]`），Judge 会以此作为判断回复实时性的时间依据。对于不涉及时间敏感性的问题，该维度记为 `-`，不参与统计。

#### 本地化评测

如果数据集提供了 `language` 和 `location` 列，Judge 会评估回复是否适配了用户的语言和位置偏好（如中国推荐微信、美国推荐 Facebook、日本推荐 LINE）。无法推断目标文化背景时，该维度记为 `-`，不参与统计。
