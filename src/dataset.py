...
Excel 约定列名：query (用户问题), api_json (messages 数组 JSON 字符串)。
api_json 中的 content 字段可以使用 {{ var_name }} Jinja2 占位符。
"""
...
def render_messages(api_json_str: str, variables: dict) -> list[dict]:
    """用 Jinja2 渲染 api_json 模板字符串，返回 messages 列表。

    Args:
        api_json_str: 包含 {{ placeholder }} 的 messages 数组 JSON 字符串
        variables: 模板变量映射（system_prompt, query 等）

    Returns:
        渲染并解析后的 messages 列表，如：
